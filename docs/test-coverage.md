...
| `Dynamic.Json.EfCore.PostgreSql` | Scalar JSON translation | Covered | Docker-free SQL tests verify text, safe decimal/date conversion, portable constant and parameterized paths, registration requirements, and unsupported-path rejection. PostgreSQL Testcontainers coverage verifies valid values plus missing, JSON-null, database-null, and invalid conversion results. |
